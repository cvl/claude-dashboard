...
8. PTY state-file matching must resolve the exact database session endpoint, not match by display name alone. This requirement is refined by the later `106afd6` review after a live moved-channel regression exposed that state-file project text can be stale.
...
8. Correct the PTY isolation implementation using the database session endpoint. Live evidence: agent `cdash` was launched with state-file project `claude-dashboard`, then moved to chat channel `cdash`; comparing `sf_project == project` made a real DM fail. Match the target session row to the state file by exact child PID (`sf_pid == r_pid`) and expected display name. The database row already scopes that PID to the selected chat project. Do not depend on state-file project text after channel moves. Add regressions for duplicate names across projects and a session moved to a differently named channel.
